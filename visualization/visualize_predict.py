...
def read_data():
    ...
    para_path = os.path.join(args.pre_folder, 'params.npy')
    pre_para = np.load(para_path, allow_pickle=True).tolist()

    print(pre_para)

    pre_partmotion = np.degrees(pre_para['part_motion'])

    # print(pre_para['obj_rot_angle'].shape)
    # pre_pitch = pre_para['obj_rot_angle'][0,1]
    # pre_yaw = pre_para['obj_rot_angle'][0,3]
    # pre_roll = pre_para['obj_rot_angle'][0,5]

    pre_x = pre_para['obj_offset'][0]
    pre_y = pre_para['obj_offset'][1]
    pre_z = pre_para['obj_offset'][2]

    pre_xdim = pre_para['obj_dim'][0]
    pre_ydim = pre_para['obj_dim'][1]
    pre_zdim = pre_para['obj_dim'][2]
    ...
    data = {'part_motion': pre_partmotion,
            'pitch': pre_pitch,
            'yaw': pre_yaw,
            'roll': pre_roll,
            'x_offset': pre_x,
            'y_offset': pre_y,
            'z_offset': pre_z,
            'obj_size': [pre_xdim, pre_ydim, pre_zdim],
            'cad': gt_cad,
            'part': gt_part,
            'focal': gt_focalX}
...
parser.add_argument("--pre_folder", type=str, help="predicted articulation data folder")
...
for index in range(num_frames):
    ...
    cv2.imwrite(os.path.join("pre_us_visual_2", f"frame{index}.png"), img_blend*255)
